				  ...
				  <?php
					...
					?>
						<tr id="i_<?php echo $items['id']; ?>">
							...
							<td><?php echo $items['telefone']; ?></td>
							<td><?php echo $items['mensagem']; ?></td>
							<td><?php echo $items['data']; ?></td>
							<td class="action">
								<?php
									echo btn_status(array("nameform"=>$nameform, "id"=>$items['id'], "title"=>status($items['status'], 'info')));
								?>
								<a href="<?=base_url('painel/contatos/upd/'.$items['id'].'/0');?>" class="btn btn-outline-primary">Editar</a>
